...
```bash
dynamo-stream$ sam build --use-container
```
...
```bash
dynamo-stream$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
dynamo-stream$ sam logs -n HelloWorldFunction --stack-name dynamo-stream --tail
```
...
```bash
dynamo-stream$ pip install pytest pytest-mock --user
dynamo-stream$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name dynamo-stream
```
